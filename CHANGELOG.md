...
### Changed
* Migrate from PHPStan to Mago for static code analysis.
...
## [7.2.0] - 2025-11-01
### Added
* Add support for redis credentials when using redis sentinels.

### Changed
* Update requirement of `symfony/lock` to `^7.3.2`, now that issues with redis cluster have been solved upstream.
...
### Deprecated
* Deprecate `QrCodeResponse` and make `endroid/qr-code` a suggested dependency instead of a mandatory one.
...
### Fixed
* Support `symfony/lock` 7.1 again, as there are bugs in 7.2 that require a downgrade until it's been fixed
